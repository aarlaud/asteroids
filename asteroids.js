...
class Ship {
    ...
    Rotate(dir){
        this.rotationalVel = (this.rotateSpeed * dir);
    }
    Update(){
        this.angle += this.rotationalVel;
        this.rotationalVel *= 0.99;
        let radians = this.angle / Math.PI * 180;
        if(this.movingForward){
            this.velX = this.velX + (Math.cos(radians) * this.speed);
            this.velY = this.velY + (Math.sin(radians) * this.speed);
        }
...
function Render() {
    ...
    if(asteroids.length <= 0){
        ctx.font = '50px Arial';
        ctx.fillText('WELL DONE!', canvasWidth / 2 - 150,canvasHeight / 2);
    }
